...
Output JSON format only:
{
  ...
  "inspectionChecklist": ["Check item 1", "Check item 2"]
    ...
    const userPrompt = `Target Vehicle: ${variant.year} ${variant.brand.name} ${variant.model.name} ${trimName} (${variant.engine.code}, ${variant.transmission.name}, Body Type: ${variant.bodyType}, Fuel: ${getFuelTypeTr(variant.fuelType)}, Turbocharged: ${variant.engine.hasTurbo ? 'Yes' : 'No'})
