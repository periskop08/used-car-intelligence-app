...
export class VehicleReportFallbackService {
  ...
  generateFallbackReport(
    ...
    const isCvt = /cvt|multidrive|x-tronic/i.test(`${trans} ${vIdentity.transmissionCode || ''}`);
    const isTorqueConverter = /tork konvert|tam otomatik|zf|eat8|eat6|tiptronic|steptronic|geartronic|g-tronic/i.test(`${trans} ${vIdentity.transmissionCode || ''}`);
    const isSingleClutch = /dualogic|etg|auto6r|easytronic|robotize|tek kavrama/i.test(`${trans} ${vIdentity.transmissionCode || ''}`);
    ...
    const transDisplayName = isDualClutch
      ...
      : (isCvt
        ? 'Sürekli Değişken Oranlı Otomatik (CVT)'
        : (isTorqueConverter
    ...
    const expertDecisionSynthesis: ExpertDecisionSynthesis = {
      ...
      dailyUseAssessment: {
        ...
        highwayUse: isElectric
          ...
          : (isCvt
            ? `Sabit hız otoyol sürüşlerinde ${hp} motor gücü akıcı bir seyir kararlılığı sunar; ani hızlanmalarda ise motor devrinin bağırması (rubber-band) karakteristiği gözlemlenebilir.`
        ...
        trafficBehavior: isElectric
          ...
          : (isEcvtOrHybrid
            ...
            : (isDualClutch
              ...
              : (isTorqueConverter
                ? 'Hidrolik tork konvertörü dur-kalklarda şanzımanı mekanik sürtünme yıpranmasından korur; 60.000-80.000 km periyodik yağ ve filtre bakımı ömrü uzatır.'
    ...
    return {
      ...
      vehicleIdentity: {
        ...
        powerSource: vIdentity.powerSource || (vIdentity.enginePowerHp ? 'VEHICLE_DATABASE' : undefined),
        powerSemantic: vIdentity.powerSemantic,
      ...
      performanceUsage: {
        ...
        powerUnit: vIdentity.powerUnit || perfSpecs.powerUnit || 'HP',
        powerSource: vIdentity.powerSource || perfSpecs.powerSource,
        powerSemantic: vIdentity.powerSemantic || perfSpecs.powerSemantic,
        torqueNm: vIdentity.engineTorqueNm || perfSpecs.engineTorqueNm,
        torqueUnit: vIdentity.torqueUnit || perfSpecs.torqueUnit || 'Nm',
        torqueSource: vIdentity.torqueSource || perfSpecs.torqueSource,
        torqueSemantic: vIdentity.torqueSemantic || perfSpecs.torqueSemantic,
        powerRpm: vIdentity.enginePowerHp ? `${vIdentity.enginePowerHp} ${vIdentity.powerUnit || 'HP'} @ 5500 d/dk` : undefined,
        torqueRpm: perfSpecs.engineTorqueNm ? `${perfSpecs.engineTorqueNm} ${perfSpecs.torqueUnit || 'Nm'} @ 1750-4000 d/dk` : undefined,
        ...
        supportingFactIds: vIdentity.powerSource === 'VEHICLE_DATABASE' ? ['VEHICLE_DATABASE'] : ['SYSTEM_DERIVED'],
      },
