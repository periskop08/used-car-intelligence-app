...
// Single global readline interface to prevent closing stdin stream prematurely
...
function askQuestion(query: string): Promise<string> {
  ...
  return new Promise(resolve => globalRl!.question(query, ans => {
    resolve(ans);
  }));
}
...
// Convert DB values to display values for matching
...
async function main() {
  console.log("==================================================");
  console.log("    VEHICLE DATABASE AUDIT & VERIFICATION TOOL");
  console.log("==================================================");
  ...
  // Parse Command-line arguments: --brand=Subaru --yes
  ...
  if (!country) {
    console.error("Error: Country TR not found in database. Run base seed first.");
    process.exit(1);
  }
  const countryId = country.id;

  // Let the user select the brand to audit if not provided via arguments
  if (!targetBrand) {
    console.log("\nPopüler markalar: Subaru, Audi, BMW, Volkswagen, Renault, Fiat, Ford, Opel, Peugeot, Toyota, Hyundai, Honda, Mercedes-Benz");
  ...
  console.log(`\n[1/3] CSV dosyası '${targetBrand}' için taranıyor (Yıl >= 2000)...`);
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    const modelName = fields[1];
    const bodyType = fields[2];
    const year = parseInt(fields[3]);
    const engine = fields[4];
    const fuel = fields[5];
    const trim = fields[6] || 'Standart';
    const transmission = fields[7];

    // Case-insensitive match for the target brand
    ...
    if (!uniqueKeysInCSV.has(key)) {
      ...
      csvVariants.push({
        brand: brandName, // keep original CSV casing
        model: modelName,
        bodyType,
        year,
        engine,
  ...
  console.log(` -> CSV dosyasında ${csvVariants.length} adet doğrulanmış varyant kombinasyonu bulundu.`);
  ...
  // Map DB variants to keys
  const dbKeys = new Map<string, string>(); // key -> id
  ...
  // Find missing in DB (exists in CSV but not in DB)
  ...
  const toRemove = dbVariants.filter(dv => {
    ...
    // Enforce year >= 2000 constraint (delete older ones if any exist for this brand)
    if (year < 2000) return true;

    // Delete if it is a placeholder or not present in the verified CSV list
    return !uniqueKeysInCSV.has(key);
  });

  console.log(`\n==================================================`);
  console.log(`               AUDIT SONUÇLARI (${targetBrand.toUpperCase()})`);
  console.log(`==================================================`);
  ...
  console.log(` ❌ Silinecek Yapay/Hatalı Varyantlar   : ${toRemove.length}`);
  console.log(`==================================================`);
  ...
  if (toRemove.length > 0) {
    console.log("\nSilinecek varyantlardan bazıları (Örnek):");
    toRemove.slice(0, 5).forEach(v => {
      console.log(`  - [SİL] ${v.model.name} (${v.year}) > ${v.engine.code} > ${v.trim.name} [ID: ${v.id}]`);
    });
    if (toRemove.length > 5) console.log(`  ... ve diğer ${toRemove.length - 5} kayıt.`);
  }

  if (toAdd.length > 0) {
    console.log("\nEklenecek varyantlardan bazıları (Örnek):");
    toAdd.slice(0, 5).forEach(v => {
      console.log(`  - [EKLE] ${v.model} (${v.year}) > ${v.bodyType} > ${v.engine} > ${v.fuel} > ${v.transmission} > ${v.trim}`);
    });
    if (toAdd.length > 5) console.log(`  ... ve diğer ${toAdd.length - 5} kayıt.`);
  }

  let confirm = 'yes';
  if (!autoYes) {
    const confirmInput = await askQuestion("\nYukarıdaki değişiklikleri Neon Veritabanına uygulamak istiyor musunuz? (yes/no): ");
    confirm = confirmInput.trim().toLowerCase();
  } else {
    console.log("\n--yes argümanı algılandı. Değişiklikler otomatik olarak onaylandı.");
  }
  ...
  if (toAdd.length > 0) {
    ...
    // Load existing mapping indexes to prevent duplicate creations
    ...
    for (const v of toAdd) {
      ...
      // 3. Ensure Engine exists
      let engineId = engineMap.get(v.engine.toLowerCase());
      if (!engineId) {
        const fuelType = mapFuelType(v.fuel);
        const horsepower = v.engine.toLowerCase().includes('hp') ? parseInt(v.engine.match(/(\d+)\s*hp/i)?.[1] || '110') : 110;
        const torque = Math.round(horsepower * 1.3);
        const newEngine = await prisma.engine.create({
          data: {
            code: v.engine.substring(0, 30),
            displacement: v.engine.includes('1.5') ? 1500 : 1600,
            horsepower,
            torque,
            fuelType,
            hasTurbo: v.engine.toLowerCase().includes('turbo') || v.engine.toLowerCase().includes('t')
          }
        });
