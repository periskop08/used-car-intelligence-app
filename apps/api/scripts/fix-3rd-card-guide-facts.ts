...
// Model-specific rich automotive guidance facts for Card 3 (replacing generic maintenance/history advice)
const CARD_3_REPLACEMENTS: Record<string, RichFact> = {
  // Audi
  "Audi|A3|A3 8V": {
  ...
  "Toyota|Corolla|Corolla E170": {
    titleTr: "1.4 D-4D DİZEL MOTOR VE KABİN GENİŞLİĞİ",
    descTr: "1.4 D-4D dizel motorun düşük tüketimi ve arka koltuktaki düz şaft tünelsiz zemin yapısı aile seyahatlerinde yüksek konfor vaat eder.",
    titleEn: "1.4 D-4D Diesel Fuel Economy & Cabin Room",
    descEn: "1.4 D-4D diesel efficiency and flat rear floor without a tall tunnel provide high passenger comfort for family journeys.",
    iconKey: "engine"
  },
  "Toyota|C-HR|C-HR II": {
  ...
  "TOGG|T10X|T10X": {
    titleTr: "88.5 kWh BATARYA VE UÇTAN UCA PANORAMİK EKRAN",
    descTr: "523 km WLTP menzilli uzun menzil bataryası ve konsolu kaplayan 29 inçlik yekpare ekran donanımı yerli EV deneyimini öne çıkarır.",
    titleEn: "88.5 kWh Battery & Full Panoramic Screen",
    descEn: "523 km WLTP long-range battery pack and seamless 29-inch widescreen dashboard offer a high-tech domestic EV experience.",
    iconKey: "screen"
  }
};
...
async function fix3rdCardGuideFacts() {
  console.log("====================================================");
  console.log("🚀 TARGETING & REPLACING ALL 3RD CARD FACTS WITH RICH AUTOMOTIVE INSIGHTS");
  console.log("====================================================");
  ...
  for (const card of cards) {
    // Determine 3rd fact (displayOrder 3 or index 2)
    let thirdFact = card.facts.find(f => f.displayOrder === 3);
    if (!thirdFact && card.facts.length >= 3) {
      thirdFact = card.facts[2];
    }
    if (!thirdFact && card.facts.length > 0) {
      thirdFact = card.facts[card.facts.length - 1];
    }

    if (!thirdFact) continue;

    const exactKey = `${card.brand}|${card.model}|${card.generationName}`;
    let richFact = CARD_3_REPLACEMENTS[exactKey];
    if (!richFact) {
      const foundEntry = Object.entries(CARD_3_REPLACEMENTS).find(([k]) => k.startsWith(`${card.brand}|${card.model}`));
      if (foundEntry) {
    ...
    console.log(`\nUpdating 3rd Fact for Card [${card.brand} ${card.model} - ${card.generationName}]:`);
    console.log(`  OLD 3rd Fact: ${thirdFact.title}`);
    console.log(`  NEW 3rd Fact: ${richFact.titleTr}`);

    // Update 3rd fact in DB
    await prisma.vehicleGuideFact.update({
      where: { id: thirdFact.id },
      data: {
    ...
    // Update TR translation if present
    const trTrans = thirdFact.translations.find(t => t.locale === 'tr');
    if (trTrans) {
    ...
    // Update EN translation if present
    const enTrans = thirdFact.translations.find(t => t.locale === 'en');
    if (enTrans) {
      await prisma.vehicleGuideFactTranslation.update({
        where: { id: enTrans.id },
        data: {
  ...
  console.log("\n====================================================");
  console.log(`✅ SUCCESS: Replaced 3rd fact across all ${updatedCount} VehicleGuideCards with rich automotive insights!`);
  console.log("====================================================");
}

fix3rdCardGuideFacts()
  .catch(err => {
    console.error("Fatal error updating 3rd guide card facts:", err);
    process.exit(1);
  })
  .finally(async () => {
