...
const SPECIFIC_CARD3_MAP: Record<string, RichFact> = {
  "Audi|A3|A3 8V": {
...
const SPECIFIC_CARD4_MAP: Record<string, RichFact> = {
  "Audi|A3|A3 8V": {
...
async function finalizeCard3And4Order() {
  console.log("====================================================");
  console.log("🚀 FINALIZING CARD 3 & CARD 4 EXACT LAYOUT ORDER IN DB");
  console.log("====================================================");
  ...
  console.log(`Processing ${cards.length} total VehicleGuideCards...`);
  ...
  for (const card of cards) {
    ...
    // Get specific or fallback for Card 3
    let richCard3 = SPECIFIC_CARD3_MAP[exactKey];
    if (!richCard3) {
      const found = Object.entries(SPECIFIC_CARD3_MAP).find(([k]) => k.startsWith(`${card.brand}|${card.model}`));
      richCard3 = found ? found[1] : getFallbackCard3(card.brand, card.model, card.generationName);
    }

    // Get specific or fallback for Card 4
    let richCard4 = SPECIFIC_CARD4_MAP[exactKey];
    if (!richCard4) {
      richCard4 = getFallbackCard4(card.brand, card.model, card.generationName);
    }

    const fact3 = card.facts[2]; // 3rd item
    const fact4 = card.facts[3]; // 4th item
    ...
    const trTrans3 = fact3.translations.find(t => t.locale === 'tr');
    if (trTrans3) {
      await prisma.vehicleGuideFactTranslation.update({
        where: { id: trTrans3.id },
        data: { title: richCard3.titleTr, description: richCard3.descTr }
      });
    }

    // Update 4th Fact (only if currently generic or fallback)
    if (fact4.title.includes('SÜRÜŞ VE ŞASİ KARAKTERİ') || fact4.title.includes('Geçmişi') || fact4.title.includes('Kullanıcı')) {
      await prisma.vehicleGuideFact.update({
        where: { id: fact4.id },
        data: {
      ...
      const trTrans4 = fact4.translations.find(t => t.locale === 'tr');
      if (trTrans4) {
        await prisma.vehicleGuideFactTranslation.update({
          where: { id: trTrans4.id },
  ...
  console.log("====================================================");
  console.log("✅ SUCCESS: Card 3 is now mühürlendi with rich model-specific automotive guidance across all vehicle guide cards!");
  console.log("====================================================");
}

finalizeCard3And4Order()
  .catch(err => {
    console.error("Fatal error finalizing card order:", err);
    process.exit(1);
  })
  .finally(async () => {
