  ...
  if (!restrictions || restrictions.length === 0) {
    return (
      <div className="p-6 rounded-2xl border border-white/10 bg-slate-900/60 text-center">
        ...
        <h4 className="text-xs font-bold text-slate-300">Aktif Kısıtlama Yok</h4>
    ...
    <div className="rounded-2xl border border-white/10 bg-slate-900/60 overflow-hidden">
      <div className="p-4 border-b border-white/10 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-black text-white">Aktif Kısıtlamalar</h3>
        </div>
        <Link
      ...
      <div className="divide-y divide-white/5">
        {restrictions.map((item) => (
          <div key={item.id} className="p-4 hover:bg-white/[0.02] transition flex items-center justify-between gap-4">
            <div>
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-xs font-bold text-slate-200">{item.userFormatted}</span>
              </div>
              <p className="text-[11px] text-slate-400">Neden: {item.reason}</p>
            </div>
