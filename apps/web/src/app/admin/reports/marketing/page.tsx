...
export default function MarketingReportsPage() {
  ...
  const handleImportAdSpend = async () => {
    try {
      ...
      setImportStatus(`Başarıyla ${d.importedCount || 0} harcama kaydı aktarıldı!`);
      setImportJson('');
      fetchMarketing();
    } catch (e: any) {
        ...
        <main className="flex-1 space-y-8 w-full">
          ...
          {data && (
            <div className="space-y-6">
              ...
              {/* Channel Breakdown */}
              {Array.isArray(data.channelBreakdown) && (
                    ...
                    {data.channelBreakdown.map((ch: any, idx: number) => (
                      <div key={idx} className="flex justify-between items-center p-3 bg-slate-950 rounded-xl border border-white/5">
                        <div>
                          ...
                          <span className="text-slate-500">{ch.registrations || 0} Kayıt</span>
                        </div>
                        <strong className="text-emerald-400">₺{(ch.spend || 0).toLocaleString('tr-TR')}</strong>
                      </div>
                    ))}
                  </div>
                </div>
