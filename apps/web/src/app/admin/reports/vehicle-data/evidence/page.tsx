...
export default function VehicleDataEvidencePage() {
  const [data, setData] = useState<any>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    fetchReportApi('/admin/reports/vehicle-data/evidence')
      .then((res) => {
  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-4 py-8 space-y-8">
      <ReportHeader title="Evidence (Kanıt) Güven Kalitesi" subtitle="Onaylanmış kronik sorunlar ve inceleme bekleyen ham veri kaynakları." />
