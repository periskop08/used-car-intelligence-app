...
export default function ProductChatbotPage() {
  const [data, setData] = useState<any>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    fetchReportApi('/admin/reports/product/chatbot')
      .then((res) => {
            ...
            <div className="space-y-6">
              ...
              {Array.isArray(data.topTopics) && (
                <div className="p-6 bg-slate-900/60 rounded-2xl border border-white/5 space-y-4">
                  <h3 className="text-sm font-bold text-slate-200">En Çok Sorulan Konu Başlıkları</h3>
                  <div className="space-y-2 font-mono text-xs">
                    {data.topTopics.map((t: any, idx: number) => (
                      <div key={idx} className="flex justify-between items-center p-3 bg-slate-950 rounded-xl border border-white/5">
                        <span className="text-slate-300 font-sans font-bold">{t.topic}</span>
                        <strong className="text-orange-400">{t.count || 0} Mesaj</strong>
                      </div>
                    ))}
                  </div>
                </div>
