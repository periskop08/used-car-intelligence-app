...
export default function ProductVehicleDiscoveryPage() {
  const [data, setData] = useState<any>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    fetchReportApi('/admin/reports/product/vehicle-discovery')
      .then((res) => {
  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-4 py-8 space-y-8">
      <ReportHeader title="Aracını Bul (Swipe) Analitiği" subtitle="Tinder usulü araç keşif modülünde oturum sayıları ve kaydırma etkileşimleri." />
