...
export default function IsiCepteProvidersPage() {
  ...
  // Selected Provider for Read-Only Admin Detail Drawer Modal
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto font-sans">
                    ...
                    <tr key={p.id} className="hover:bg-white/[0.03] transition">
                      ...
                      <td className="p-4">
                        <div className="flex flex-wrap gap-1 max-w-[160px]">
                          ...
                          {p.autoServiceCategories?.length > 2 && (
                      ...
                      <td className="p-4">
                        <div className="flex flex-wrap gap-1 max-w-[160px]">
                          ...
                          {p.supportedVehicleBrands?.length > 2 && (
      ...
      {/* READ-ONLY ADMIN DETAIL DRAWER MODAL */}
      {selectedProvider && (
        <div className="fixed inset-0 z-50 flex items-center justify-end bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="w-full max-w-2xl h-full bg-[#0b0f19] border-l border-white/10 p-6 space-y-6 overflow-y-auto font-sans shadow-2xl">
            {/* Modal Header */}
            <div className="flex justify-between items-start pb-4 border-b border-white/10">
              <div>
                ...
                <h2 className="text-lg font-black text-white mt-1">{selectedProvider.businessName}</h2>
                <span className="text-[10px] text-slate-400 font-mono block mt-0.5">
                  Veri Kaynağı: {selectedProvider.sourceSystem || 'İşi Cepte Sync'}
                </span>
              </div>
              <button
            ...
            <div className="p-3 bg-cyan-500/10 border border-cyan-500/20 rounded-xl text-[11px] text-cyan-300 flex items-center gap-2">
              ...
              <span>
                İşletme profili, üyelik ve kategori bilgileri ana veri kaynağı olan İşi Cepte sisteminden senkronize edilir. TorqueScout içerisinden düzenleme yapılmaz.
              </span>
            </div>

            {/* Section A: Temel Kimlik */}
            <div className="p-4 bg-slate-950 rounded-2xl border border-white/5 space-y-2 font-mono text-xs">
              <span className="text-[10px] uppercase font-bold text-slate-500 block">Section A • Temel Kimlik</span>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <span className="text-slate-500 block text-[10px]">İşi Cepte Primary ID</span>
                  <span className="text-white font-bold">{selectedProvider.isicepteProviderId}</span>
                </div>
                <div>
                  <span className="text-slate-500 block text-[10px]">TorqueScout Ref ID</span>
                  <span className="text-slate-300">{selectedProvider.id}</span>
                </div>
              </div>
            </div>

            {/* Section B: Üyelik & TorqueScout Listeleme */}
            <div className="p-4 bg-slate-950 rounded-2xl border border-white/5 space-y-3 text-xs">
              <span className="text-[10px] uppercase font-bold text-slate-500 font-mono block">Section B • Üyelik ve Listeleme Durumu</span>
              <div className="grid grid-cols-3 gap-3 font-mono">
                <div>
                  <span className="text-slate-500 block text-[10px]">İşi Cepte Üyeliği</span>
                  <span className="text-emerald-400 font-bold">{selectedProvider.membershipStatus}</span>
                </div>
                <div>
                  <span className="text-slate-500 block text-[10px]">TS Opt-In Tercihi</span>
                  <span className="text-cyan-400 font-bold">{selectedProvider.torqueScoutOptIn ? 'Opt-In (Evet)' : 'Opt-Out (Hayır)'}</span>
                </div>
                <div>
                  <span className="text-slate-500 block text-[10px]">Lokal Görünürlük State</span>
                  <span className="text-slate-300">{selectedProvider.localListingState || 'ELIGIBLE'}</span>
                </div>
              </div>
            </div>

            {/* Section C: Coğrafi Bölge */}
            <div className="p-4 bg-slate-950 rounded-2xl border border-white/5 space-y-2 text-xs">
              <span className="text-[10px] uppercase font-bold text-slate-500 font-mono block">Section C • Coğrafi Hizmet Bölgesi</span>
              <div className="grid grid-cols-3 gap-3 font-mono">
                <div>
                ...
                <div>
                  <span className="text-slate-500 block text-[10px]">İl / Bölge Kodu</span>
                ...
                <div>
                  ...
                  <span className="text-white font-bold">{selectedProvider.district || 'Tüm İl'}</span>
                </div>
              </div>
            </div>

            {/* Section D: Oto Hizmet Kategorileri */}
            <div className="p-4 bg-slate-950 rounded-2xl border border-white/5 space-y-2 text-xs">
              <span className="text-[10px] uppercase font-bold text-slate-500 font-mono block">Section D • Senkronize Oto Hizmet Kategorileri</span>
              <div className="flex flex-wrap gap-2">
                {selectedProvider.autoServiceCategories?.map((c) => (
                  <span key={c.id} className="px-2.5 py-1 bg-slate-900 border border-white/10 text-cyan-300 rounded-lg text-xs font-medium">
                    {c.name} ({c.id})
                  </span>
                ))}
              </div>
            ...
            {/* Section E: Hizmet Verilen Araç Markaları */}
            <div className="p-4 bg-slate-950 rounded-2xl border border-white/5 space-y-2 text-xs">
              <div className="flex justify-between items-center">
                <span className="text-[10px] uppercase font-bold text-slate-500 font-mono">Section E • Desteklenen Araç Markaları</span>
                <span className="text-[9px] text-slate-500 italic">Genel servis uyumluluğu</span>
              </div>
              <div className="flex flex-wrap gap-2">
                {selectedProvider.supportedVehicleBrands?.map((b, idx) => (
                  <span key={idx} className="px-2.5 py-1 bg-slate-900 border border-white/10 text-purple-300 rounded-lg text-xs font-medium">
                    {b.name}
                  </span>
                ))}
              </div>
            ...
            {/* Section F: Görünürlük Hakları (Entitlements) */}
            <div className="p-4 bg-slate-950 rounded-2xl border border-white/5 space-y-3 text-xs">
              <span className="text-[10px] uppercase font-bold text-slate-500 font-mono block">Section F • Görünürlük Hakları (Vitrin & Ülke Geneli)</span>
                ...
                <div className="p-3 bg-slate-900 border border-amber-500/20 rounded-xl space-y-1.5">
                  <div className="flex justify-between items-center">
                    ...
                    <span className={`text-[10px] font-mono font-bold px-1.5 py-0.5 rounded ${selectedProvider.showcase?.active ? 'bg-amber-500/20 text-amber-300' : 'bg-slate-800 text-slate-500'}`}>
                      {selectedProvider.showcase?.active ? 'AKTİF' : 'YOK'}
                    </span>
                  </div>
                  {selectedProvider.showcase?.active && (
                    <div className="text-[10px] text-slate-400 font-mono space-y-0.5 pt-1 border-t border-white/5">
                      <div>Kaynak: {selectedProvider.showcase.source || 'İşi Cepte'}</div>
                ...
                <div className="p-3 bg-slate-900 border border-purple-500/20 rounded-xl space-y-1.5">
                  <div className="flex justify-between items-center">
                    ...
                    <span className={`text-[10px] font-mono font-bold px-1.5 py-0.5 rounded ${selectedProvider.nationalVisibility?.active ? 'bg-purple-500/20 text-purple-300' : 'bg-slate-800 text-slate-500'}`}>
                      {selectedProvider.nationalVisibility?.active ? 'AKTİF' : 'YOK'}
                    </span>
                  </div>
                  {selectedProvider.nationalVisibility?.active && (
                    <div className="text-[10px] text-slate-400 font-mono space-y-0.5 pt-1 border-t border-white/5">
                      <div>Kaynak: {selectedProvider.nationalVisibility.source || 'İşi Cepte'}</div>
            ...
            {/* Section G: Senkronizasyon Zaman Çizelgesi */}
            <div className="p-4 bg-slate-950 rounded-2xl border border-white/5 space-y-2 text-[11px] font-mono text-slate-400">
              <span className="text-[10px] uppercase font-bold text-slate-500 block">Section G • Senkronizasyon Kayıt Zamanı</span>
              <div className="flex justify-between items-center">
