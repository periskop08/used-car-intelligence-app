...
import { IsiCepteProvider, IsiCepteEntitlementSource } from '@/types/isiCepteDomain';
...
export default function IsiCepteNationalVisibilityPage() {
  const [records, setRecords] = useState<IsiCepteNationalVisibilityRecord[]>([]);
  ...
  const fetchNationalData = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/admin/isi-cepte/national-visibility?${query.toString()}`, {
        headers: {
      ...
      console.error('National visibility fetch error:', err);
      setRecords([]);
      setTotalCount(0);
      setTotalPages(1);
    } finally {
    ...
    <div className="space-y-6 max-w-7xl mx-auto font-sans relative">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-6 border-b border-white/10">
        <div>
          ...
          <p className="text-xs text-slate-400 font-medium mt-1">
            Hizmet bölgesinin ötesinde Türkiye geneli TorqueScout görünürlük hakkı edinen İşi Cepte üyeleri
            <span className="text-purple-400 font-bold font-mono ml-2">(Toplam {totalCount} Ülke Geneli kaydı)</span>
          </p>
        </div>
        <button
      ...
      <div className="p-4 bg-purple-500/10 border border-purple-500/20 rounded-2xl flex items-start gap-3 text-xs text-purple-300">
        ...
        <div className="space-y-1">
          ...
          <p className="text-slate-300 leading-relaxed">
            Ülke Geneli hakkı, işletmenin coğrafi görünürlük alanını ülke çapına genişletir. Ancak üyelik, opt-in, marka uyumluluğu ve hizmet kategorisi şartlarını devreden çıkarmaz.
          </p>
        </div>
      </div>
      ...
      <div className="p-4 bg-slate-900/60 rounded-2xl border border-white/5 space-y-3">
        <div className="flex flex-wrap gap-3 items-center justify-between">
          <div className="flex flex-1 min-w-[260px] items-center gap-2 px-3.5 py-2 bg-slate-950 rounded-xl border border-white/10 focus-within:border-purple-500/50 transition">
          ...
          <div className="flex flex-wrap items-center gap-2 text-xs font-mono">
            ...
            >
              ...
              <option value="ACTIVE">Aktif Ülke Geneli</option>
      ...
      <div className="bg-slate-900/60 rounded-2xl border border-white/5 overflow-hidden">
        {loading ? (
          <div className="p-12 text-center text-slate-400 font-medium text-xs flex items-center justify-center gap-2">
            ...
            <span>Ülke Geneli görünürlük kayıtları yükleniyor...</span>
          </div>
        ) : records.length === 0 ? (
          <div className="p-12 text-center space-y-3">
            ...
            <p className="text-xs text-slate-400 max-w-md mx-auto leading-relaxed">
              İşi Cepte üzerinden oluşturulan Ülke Geneli hakları sisteme aktarıldığında burada görüntülenecektir.
            </p>
          </div>
        ) : (
          <div>
              ...
              <table className="w-full text-left text-xs">
                <thead className="bg-slate-950/80 text-slate-400 font-bold uppercase tracking-wider text-[10px] border-b border-white/5 font-mono">
                  <tr>
                    ...
                    <th className="p-4">Fiziksel Bölge</th>
                    <th className="p-4">Ülke Geneli Durumu</th>
                  ...
                  {records.map((r) => (
                    <tr
                      ...
                      className={`hover:bg-white/[0.04] transition cursor-pointer ${
                        selectedProvider?.id === r.provider?.id ? 'bg-purple-500/10 border-l-2 border-l-purple-400' : ''
                      }`}
                    >
                      ...
                      <td className="p-4 text-right">
                        <button
                          ...
                          className="px-3 py-1.5 bg-slate-800 hover:bg-purple-500/20 text-slate-300 hover:text-purple-300 border border-white/10 hover:border-purple-500/30 rounded-lg text-[11px] font-bold transition cursor-pointer"
                        >
                          İncele →
                        </button>
                      </td>
            ...
            {totalPages > 1 && (
              <div className="p-4 border-t border-white/5 flex items-center justify-between text-xs font-mono">
                <span className="text-slate-400">
                  Sayfa <strong className="text-white">{page}</strong> / {totalPages} (Toplam {totalCount} Ülke Geneli kaydı)
                </span>
                <div className="flex items-center gap-2">
                  <button
